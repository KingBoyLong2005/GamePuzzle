...
{
    // Canvas    canvas;
    public GameObject rootUIManager;

    public Button PlayBtn;
    // void Awake() => BuildUI();
    private void Awake()
    {
        // BuildDecorIcon(rootUIManager);
        PlayBtn.onClick.AddListener(() => GameManager.Instance.ShowLevelSelect());       
    }

    public void Show() => rootUIManager?.SetActive(true);
    public void Hide() => rootUIManager?.SetActive(false);
...
//             new Color(0.15f, 0.60f, 0.95f), () => GameManagerTest.Instance.ShowLevelSelect());
