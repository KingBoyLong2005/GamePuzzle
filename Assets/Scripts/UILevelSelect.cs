...
public class UILevelSelect : MonoBehaviour
{
    ...
    void RefreshCards(LevelDataSO[] levels)
    {
        ...
        for (int i = 0; i < levels.Length; i++)
        {
            ...
            var card    = Instantiate(cardPrefab,spawnCard);
            // card.transform.SetParent(gridContainer.transform, false);
            card.GetComponent<UICardSelectedLevel>().BuildCard(card, level, i + 1);
            // card.GetComponentInChildren<UICardSelectedLevel>().BuildCard(card, level, i + 1, done);
            // GetComponentInChildren<UICardSelectedLevel>().BuildCard(card, level, i + 1, done);
            // Instantiate(cardPrefab,spawnCard); 
            cards.Add(card);
        }
    }
...
//             new Color(0.20f, 0.25f, 0.38f), () => GameManagerTest.Instance.ShowMainMenu());
...
//         btn.navigation = nav;
//         btn.onClick.AddListener(() => GameManagerTest.Instance.StartLevel(level));
